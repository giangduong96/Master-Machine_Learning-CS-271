...
#include <time.h>
int randomSub = 0;
// question 11 part c, generating the digraph
void generate_diGraph(double A[][N], struct stepStruct *step, int T){
    for (int i = 0; i < 26; i++){
        for (int j = 0; j < 26; j++){
            A[i][j] = 0.0;
        }
...
//    sprintf(fname, argv[1]);
    ...
    srandom(seed);
     int maxMatch = 0; // count how many match
     int maxHMM = 0;  // count Hmm number
     int arrayIter[] = {1,10, 100, 1000};

     for(int k = 0; k < 4; k++) {
         for (int hmmNo = 0; hmmNo < arrayIter[k]; hmmNo++) {
             // create a new pi, B
             initMatrices(pi, B, seed);

             // create a cipher text from file
             T = GetObservations(fname, step, T,startPos, 1000, maxChars, 1);
             // in this case we need to observation sequence to 1,000 chacracters.
             //T = 1000;
             T = 1000;
             T = maxChars;
             ...
             while ((iter < maxIters)) {
                 ...
                 // do not reestimate for A instead using final B to determine a putative key
 ...
 */
             ...
             // Calculating the Putative Key part of the problem -part d
             int count = 0;
             for (int i = 0; i < N; i++) {
                 double max = B[i][0];
                 int element = 0;
                 ...
                 printf("\n%c element --> %c.", i + 97, (element + 97));
                 if (element == ((i + randomSub) % 26))
                     count++;
             }
             printf("\n The key is :%d", randomSub);
             printf("\n Final Count value is : %d", count);
             ...
             printf("\n HMM number is %d and match count %d.", hmmNo, maxMatch);
         }// end for
     }// end for
     printf("\n Max match was of HMM %d with match count %d", maxHMM, maxMatch);
}// end hmm
        ...
        for(i = thisStartPos; i <= len; ++i)
        {
            ...
            for(j = 0; j < M; ++j)
            {
                if(strncasecmp(&temp[i], &alphabet[j], 1) == 0)
                {
                    ...
                    if(totalNum >= startChar)
                    {
                        // encrypt the plainetext using randomly generated shift of alphabet
                        if (flag ==1)
                            step[num].obs = (j + randomSub) % 26;
                        else
                            step[num].obs = j;
#ifdef PRINT_OBS
